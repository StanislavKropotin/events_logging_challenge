...
1. **Outbox Pattern**: Этот паттерн позволяет записывать события в базу данных с использованием транзакций, что помогает избежать потери данных при ошибках или сбоях системы. Мы сохраняем все события в таблице `OutboxEvent`, и задача Celery будет периодически их публиковать в ClickHouse.

2. **Celery**: Мы используем Celery для асинхронной обработки событий, что позволяет не блокировать основной поток выполнения приложения и обрабатывать события параллельно.

3. **Тестирование**: Написаны тесты для всех новых компонентов системы.

4. Настроен GitHub Actions для автоматического тестирования и деплоя. Все изменения, вносимые в ветку main, автоматически проходят через процесс сборки и тестирования.

5. Запуск приложения остался без изменений
